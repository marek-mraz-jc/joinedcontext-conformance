...
| `FORGE_ADMIN_TOKEN` | a `read:repository` token of the forge's administrator, which reads the branch rule the Portal's identity may not (PF-105); without it that case skips |
